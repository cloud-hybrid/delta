...
import { default as Accordion, AccordionItem } from "../Accordion/index.js";
...
const getPopoverLabelId = (string) => `${ namespace }-popover__label--${ string }`;
...
const renderPopover = (children, state) => (
    <Transition className={ namespace } component="span">
        { state && <div className={ `${ namespace }-popover` }>{ children }</div> }
    </Transition>
);
...
export default class Header extends Component {
    ...
    renderProfile() {
        ...
        const accountElement = hasAccount && (
            <Fragment>
                <div className={ `${ namespace }-popover__profile__body__label` }>
                    { labels.profile.account }:{ " " }
                    <span className="selectable-text">{ profile.account.id }</span>
                </div>
                <div className={ `${ namespace }-popover__profile__body__name` }>
          <span className={ `${ namespace }-popover__profile__body__name__text` }>
            { hasAccount.name }
          </span>
                </div>
            </Fragment>
        ...
        return renderPopover(
            <div
                ...
                className={ `${ namespace }-popover--focus` }
            ...
            >
                <HeaderPopoverHeader
                    className={ `${ namespace }-popover__profile-shell-header` }
                >
                    <Fragment>
                        <ProfileImage
                            className={ `${ namespace }-popover__profile-shell-header__icon` }
                            profile={ profile }
                            large
                        />
                        <div>
              <span
                  id={ popoverLabelId }
                  className={ `${ namespace }-popover__profile-shell-header__title` }
              >
                { profile.name.first_name } { profile.name.surname }
              </span>
                            <span className={ `${ namespace }-popover__profile-shell-header__email` }>
                { profile.email }
              </span>
                        </div>
                    </Fragment>
                ...
                { profile.description && (
                    <section className={ `${ namespace }-popover__profile__description` }>
                        { profile.description }
                    </section>
                ) }
                { hasAccount && (
                    <section
                        className={ classnames(`${ namespace }-popover__profile__body`, {
                            [`${ namespace }-popover__profile__body--account`]:
                                !hasAccountList
                        }) }
                    >
                        { !hasAccountList ? (
                            ...
                            <Accordion
                                className={ `${ namespace }-popover__profile__body__accordion` }
                            >
                                <AccordionItem
                                    className={ `${ namespace }-popover__profile__body__accordion__item` }
                                    title={ accountElement }
                                >
                                    <ul className={ `${ namespace }-popover__profile__body__list` }>
                                        { accountList.map(({ id, name }) => (
                                            <li
                                                key={ id }
                                                className={ `${ namespace }-popover__profile__body__account` }
                                            >
                                                <button
                                                    value={ id }
                                                    onClick={ onAccountClick }
                                                    className={ `${ namespace }-popover__profile__body__account__button` }
                                                    type="button"
                                                >
                                                    { name }
                                                </button>
                ...
                <section className={ `${ namespace }-popover__footer` }>
                    { this.props.showEditProfile && (
                        <HeaderPopoverLinkSecondary
                            className={ `${ namespace }-popover__profile__footer__edit` }
                    ...
                    <HeaderPopoverLinkSecondary
                        className={ `${ namespace }-popover__profile__footer__logout` }
                        href={ links.sign_out }
                    >
    ...
    renderNotifications() {
        ...
        return renderPopover(
            ...
            >
                ...
                />
                { length > 0 && (
                    <div className={ `${ namespace }-popover__content` }>
            <span className={ `${ namespace }-popover__label` }>
              { labels.notifications.today }
            </span>
                        <button
                            className={ `${ namespace }-popover__button` }
                ...
                <ul className={ `${ namespace }-popover__list` }>
                    <Transition
                        ...
                        { notifications
                            ...
                            .map(
                                (
                                    ...
                                    <li key={ id } className={ `${ namespace }-popover__list-item` }>
                                        <HeaderNotification
                ...
                { !length && (
                    <span className={ `${ namespace }-popover__container` }>
            { labels.notifications.success }
          </span>
                ) }

                <div className={ `${ namespace }-popover__footer` }>
                    ...
                    { links.notifications_preferences && (
                        <Icon
                            className={ `${ namespace }-popover__icon` }
    ...
    render() {
        ...
        const classes = classnames("io-shell-header-container", className);

        const domainElement = (title) => (
            <span className={ `${ namespace }-link__title--domain` }>{ title }</span>
        );

        return (
            <div className={ classes }>
                <header className={ namespace } role="banner">
                    <Link className={ `${ namespace }-link` } href={ link }>
                        { `${ company } ` }
                        { domain ? (
                            <Fragment>
                                { domainElement(`${ domain } `) }
                                <span className={ `${ namespace }-link__title--product` }>
                  { product }
                </span>
                            </Fragment>
                        ) : (
