/***
 * @typedef {{host: String, path: String, method: String, port: String}} Input
 * @type {{rejectUnauthorized: boolean, path, agent: boolean, method, port, requestCert: boolean, host}} Definition
 */
...
/***
 *
 * Schema Configuration Object - Used to download & install or validate against Remote HTTPs-based Schemas.
 *
 * Please note, unless the file path defined in the `path` URI contains a JSON extension, the
 * remote query will be rejected.
 *
 * @param host {string} Webserver Hostname (Top-Level Domain-Name)
 * @param port {number} Port Number ::: Defaults to 443
 * @param path {string} Webserver Path; Directory or Filename
...
export const Configuration = (host, path, method = "GET", port = 443) => {
    const Validation = new RegExp(/.*(json)$/i);

    (Validation.exec(path) !== null) || Error("Missing JSON File Extension");
